    ...
    try {
        ...
        if (req.path.endsWith('/Authenticate')||
            ...
            if (authorizationHeader && authorizationHeader.startsWith('Bearer ')) {
                ...
                console.log(token)
                const response = await fetch(process.env.GAS_BASE_URL + '/auth/validate-token', {
                    method: 'POST',
                    body: JSON.stringify({
                        "token": token
                    }),
                ...
                res.status(401).json({ message: 'Unauthorized: No Bearer token found' });
            }
        }
...
export const refreshTokenGAS = async (Token) => {
    ...
    console.log(Token)
    try {
        const response = await fetch(process.env.GAS_BASE_URL + '/auth/refresh-token', {
            method: 'POST',
            body: JSON.stringify({
                "token": Token
            }),
